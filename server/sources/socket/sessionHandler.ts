...
/// CodeCAT sometimes ships a phase event before its in-memory snapshot
...
/// NOT use `meta.title` here because CodeCAT sets that to a "smart title"
...
export function registerSessionHandler(
    ...
    // CodeCAT acknowledges that it successfully consumed a blob, so the server
    // can drop it from disk immediately. No ack from CodeCAT = TTL sweeper handles it.

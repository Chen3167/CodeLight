...
CodeLight is a native Swift iOS app for monitoring and controlling Claude Code sessions remotely. It works with CodeIsland (macOS notch companion) and CodeLight Server (Fastify/TypeScript backend) to provide real-time session sync, E2E encrypted messaging, and iPhone Dynamic Island status display.

### Naming

| Component | Role |
|-----------|------|
| **CodeIsland** | macOS notch app (existing), gains Socket.io uplink |
...
- **CodeIsland as sole middleware** — replaces happy-cli, serves both local notch display and remote sync
...
| `rpc-call` | client → server → client | RPC request forwarding (phone → server → CodeIsland) |
...
### Shared Swift Packages (used by both CodeIsland and CodeLight)
...
- Tauri desktop → CodeIsland covers this
...
```
CodeIsland (Mac)                    CodeLight (iPhone)
     │                                    │
...
| **Phase 3** | CodeIsland additions — socket uplink, message relay, RPC | CodeIsland syncs to server |
...
- One CodeIsland instance connects to one server at a time (configurable)
